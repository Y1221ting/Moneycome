...
public class AppController {
    ...
    public JsonResult registerSimple(String mobile, String password, String realName) {
        LambdaQueryWrapper<User> qw = Wrappers.lambdaQuery();
        qw.eq(User::getMobile, mobile);
        if (this.userService.count(qw) > 0) {
            return JsonResult.failure("此手机号已被注册！");
        }

        User user = new User();
        user.setMobile(mobile);
        user.setRealName(realName != null ? realName : mobile);
        user.setPassword(DigestUtils.sha256Hex(password));
        this.userService.save(user);

        return JsonResult.successful("注册成功，请登录");
    }
